...
function c100100503.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoDeck(tc,nil,0,REASON_EFFECT)>0 and sumtype&SUMMON_TYPE_FUSION==SUMMON_TYPE_FUSION and ct>0 
		and (not Duel.IsPlayerAffectedByEffect(tp,59822133) or ct==1) and ct<=Duel.GetLocationCount(tp,LOCATION_MZONE)
		and mg:FilterCount(aux.NecroValleyFilter(c100100503.mgfilter),nil,e,tp,tc,mg)==ct and Duel.SelectYesNo(tp,aux.Stringid(100100503,0)) then
		Duel.BreakEffect()
