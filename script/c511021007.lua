...
function c511021007.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_PRE_DAMAGE_CALCULATE)
	e1:SetCondition(c511021007.condition)
	e1:SetTarget(c511021007.target)
	e1:SetOperation(c511021007.activate)
	c:RegisterEffect(e1)
end
function c511021007.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c511021007.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local g=e:GetLabelObject()
	if chk==0 then return g:GetCount()>0 end
	e:SetLabelObject(nil)
	Duel.SetTargetCard(g)
end
function c511021007.activate(e,tp,eg,ep,ev,re,r,rp)
