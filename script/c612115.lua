...
function c612115.rmfilter(c)
	return c:IsAbleToRemove() and aux.SpElimFilter(c)
end
function c612115.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE+LOCATION_GRAVE) and chkc:IsControler(1-tp) and c612115.rmfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c612115.rmfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectTarget(tp,c612115.rmfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,1,1,nil)
...
function c612115.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ft>0 and Duel.IsExistingMatchingCard(c612115.filter,tp,LOCATION_DECK,0,1,nil,e,tp) end
