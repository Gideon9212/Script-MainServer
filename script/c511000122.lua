...
function c511000122.costfilter(c,code,ft)
...
function c511000122.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ft>-1 and Duel.IsExistingMatchingCard(c511000122.costfilter,tp,LOCATION_SZONE,0,1,nil,511000125)
		and Duel.IsExistingMatchingCard(c511000122.costfilter,tp,LOCATION_SZONE,0,1,nil,511000123) 
		and Duel.IsExistingMatchingCard(c511000122.costfilter,tp,LOCATION_MZONE,0,1,nil,511000126,ft) end
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g3=Duel.SelectMatchingCard(tp,c511000122.costfilter,tp,LOCATION_MZONE,0,1,1,nil,511000126,ft)
	g1:Merge(g2)
	g1:Merge(g3)
...
function c511000122.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c511000122.spfilter,tp,0x13,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,0x13)
end
function c511000122.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 or not e:GetHandler():IsRelateToEffect(e) then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local tc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c511000122.spfilter),tp,0x13,0,1,1,nil,e,tp):GetFirst()
	if tc then
