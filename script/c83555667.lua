...
function c83555667.filter(c,lp)
	return c:IsFaceup() and c:IsAttackBelow(lp)
end
function c83555667.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local lp=Duel.GetLP(1-tp)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and c83555667.filter(chkc,lp) end
	if chk==0 then return Duel.IsExistingTarget(c83555667.filter,tp,0,LOCATION_MZONE,1,nil,lp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c83555667.filter,tp,0,LOCATION_MZONE,1,1,nil,lp)
