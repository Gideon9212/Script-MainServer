...
function c511000223.filter1(c,tp,mc)
	local lv=c:GetLevel()
	return lv>0 and c:IsFaceup() and Duel.IsExistingMatchingCard(c511000223.filter2,tp,LOCATION_HAND,0,1,nil,lv,mc)
end
function c511000223.filter2(c,lv,mc)
...
function c511000223.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(1-tp,LOCATION_MZONE)>0 
		and Duel.IsExistingTarget(c511000223.filter1,tp,0,LOCATION_MZONE,1,nil,tp,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,c511000223.filter1,tp,0,LOCATION_MZONE,1,1,nil,tp,e:GetHandler())
...
function c511000223.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local sc=Duel.SelectMatchingCard(tp,c511000223.filter2,tp,LOCATION_HAND,0,1,1,nil,tc:GetLevel(),e:GetHandler()):GetFirst()
	if sc then
