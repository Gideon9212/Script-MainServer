...
function c10960419.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToBattle() then
		...
		if Duel.Remove(tc,0,REASON_EFFECT+REASON_TEMPORARY)~=0 then
			tc:SetTurnCounter(0)
			tc:RegisterFlagEffect(10960419,RESET_EVENT+0x1fe0000,0,0)
			local e1=Effect.CreateEffect(c)
			...
			e3:SetOperation(c10960419.reset)
			e3:SetReset(RESET_PHASE+PHASE_STANDBY,ct)
			c:RegisterEffect(e3)
		end
	end
end
function c10960419.reset(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetLabelObject() then e:Reset() return end
	c10960419.turnop(e:GetLabelObject(),tp,eg,ep,ev,e,r,rp)
end
function c10960419.turncon(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject()
	return tc:GetFlagEffect(10960419)~=0
end
