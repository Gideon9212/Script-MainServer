...
function c511002892.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c511002892.filter(chkc,rg) end
	if chk==0 then return g:GetCount()>0 and Duel.IsExistingTarget(c511002892.filter,tp,LOCATION_MZONE,0,1,nil,rg) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c511002892.filter,tp,LOCATION_MZONE,0,1,1,nil,rg)
