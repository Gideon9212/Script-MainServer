...
function c511001694.filter(c,tp)
	local re=c:GetReasonEffect()
	return (Duel.GetLocationCount(tp,LOCATION_MZONE)>0 or c:GetSequence()<5) and c:GetLevel()==12 and c:IsSetCard(0xe6)
...
function c511001694.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c511001694.filter,1,nil,tp) end
	local g=Duel.SelectReleaseGroup(tp,c511001694.filter,1,1,nil,tp)
