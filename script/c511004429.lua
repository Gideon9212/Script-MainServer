...
function c511004429.target(e,tp,eg,ev,ep,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and not g:IsContains(chkc) and c511004429.filter(chkc) end
	if chk==0 then return d and g:IsExists(Card.IsControler,1,nil,1-tp)
		and Duel.IsExistingTarget(c511004429.filter,tp,LOCATION_MZONE,0,1,g) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c511004429.filter,tp,LOCATION_MZONE,0,1,1,g)
end
