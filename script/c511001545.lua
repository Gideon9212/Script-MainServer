...
function c511001545.sumtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return mg:GetCount()>0 and ft>=mg:GetCount() 
		and mg:FilterCount(c511001545.mgfilter,nil,e,tp,c)==mg:GetCount() end
...
function c511001545.sumop(e,tp,eg,ep,ev,re,r,rp)
	...
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if ft>1 and Duel.IsPlayerAffectedByEffect(tp,59822133) then ft=1 end
	if mg:GetCount()<=ft and mg:FilterCount(aux.NecroValleyFilter(c511001545.mgfilter),nil,e,tp,c)==mg:GetCount() then
