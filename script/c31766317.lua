...
function c31766317.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return eg:GetCount()==1 and dc~=e:GetHandler() and dc:IsFaceup() and dc:IsLocation(LOCATION_MZONE) 
		...
		and Duel.IsExistingMatchingCard(c31766317.rfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,nil) end
	if Duel.SelectEffectYesNo(tp,e:GetHandler(),96) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		local g=Duel.SelectMatchingCard(tp,c31766317.rfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,1,nil)
