...
function c511015114.xyzfilter(c,m,cc)
	...
	local f,lv,ct,alterf,desc,maxct,op,mustbemat=c.xyz_parameters
	...
	e1:SetCondition(aux.XyzCondition(f,lv,ct,maxct,mustbemat))
	e1:SetTarget(aux.XyzTarget(f,lv,ct,c.maxct,mustbemat))
	e1:SetOperation(aux.XyzOperation(f,lv,ct,maxct,mustbemat))
...
function c511015114.xyzop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		local f,lv,ct,alterf,desc,maxct,op,mustbemat=tc.xyz_parameters
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_FIELD)
		e1:SetDescription(1073)
		e1:SetCode(EFFECT_SPSUMMON_PROC)
		...
		e1:SetRange(LOCATION_GRAVE)
		e1:SetCondition(aux.XyzCondition(f,lv,ct,maxct,mustbemat))
		e1:SetTarget(aux.XyzTarget(f,lv,ct,maxct,mustbemat))
		e1:SetOperation(aux.XyzOperation(f,lv,ct,maxct,mustbemat))
		e1:SetValue(SUMMON_TYPE_XYZ)
		e1:SetReset(RESET_CHAIN)
