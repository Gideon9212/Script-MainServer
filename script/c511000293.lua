...
function c511000293.initial_effect(c)
	...
	if not c511000293.global_check then
		...
		ge2:SetOperation(c511000293.archchk)
		Duel.RegisterEffect(ge2,0)
	end
end
function c511000293.archchk(e,tp,eg,ep,ev,re,r,rp)
...
function c511000293.filter(c,e,tp,rank,mc)
	local rk=c:GetRank()
	return c:IsC() and rk==rank+1 and Duel.GetLocationCountFromEx(tp,tp,mc,c)>0 
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_XYZ,tp,false,false)
end
function c511000293.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local at=Duel.GetAttackTarget()
	if chk==0 then
		return at and at:GetRank()>0 and Duel.IsExistingMatchingCard(c511000293.filter,tp,LOCATION_EXTRA,0,1,nil,e,tp,rank,at)
	end
...
function c511000293.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c511000293.filter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc:GetRank(),tc)
	local sc=g:GetFirst()
	if sc then
