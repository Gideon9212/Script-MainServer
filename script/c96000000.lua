--Destiny Draw
--Scripted by Edo9300
function c96000000.initial_effect(c)
	--activate
	...
	e1:SetCountLimit(1)
	e1:SetRange(0xff)
	e1:SetCondition(c96000000.con)
	e1:SetOperation(c96000000.op)
	c:RegisterEffect(e1)
	--Destiny Draw
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(96000000,0))
	e2:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_PREDRAW)
	e2:SetRange(LOCATION_REMOVED)
	e2:SetCountLimit(1,96000000+EFFECT_COUNT_CODE_DUEL)
	e2:SetCondition(c96000000.drcon)
	e2:SetTarget(c96000000.drtg)
	e2:SetOperation(c96000000.drop)
	c:RegisterEffect(e2)
	--protection
...
function c96000000.con(e,tp,eg,ep,ev,re,r,rp)
    return Duel.GetTurnCount()==1
end
function c96000000.op(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_REMOVED,0,1,nil,96000000) then
...
function c96000000.drcon(e,tp,eg,ep,ev,re,r,rp)
...
function c96000000.drtg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c96000000.drop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
