...
function c511002981.filter2(c,e,tp,m,f)
...
function c511002981.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local res=Duel.IsExistingMatchingCard(c511002981.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c511002981.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf)
			end
		end
		return res
	end
...
function c511002981.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g4=Duel.GetFieldCard(1-tp,LOCATION_SZONE,7)
	local mg1=Group.FromCards(g1,g2,g3,g4):Filter(c511002981.filter1,nil,e)
	local sg1=Duel.GetMatchingGroup(c511002981.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
