...
function c57728571.hgfilter(c)
	return not c:IsPublic() or c57728571.filter(c)
end
function c57728571.fgfilter(c)
	return c:IsFacedown() or c57728571.filter(c)
end
function c57728571.tgfilter(c)
	return ((c:IsLocation(LOCATION_HAND) and c:IsPublic()) or (c:IsLocation(LOCATION_MZONE) and c:IsFaceup())) and c57728571.filter(c)
end
function c57728571.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c57728571.hgfilter,tp,0,LOCATION_HAND,1,nil)
		or Duel.IsExistingMatchingCard(c57728571.fgfilter,tp,0,LOCATION_MZONE,1,nil) end
	local g=Duel.GetMatchingGroup(c57728571.tgfilter,tp,0,LOCATION_MZONE+LOCATION_HAND,nil)
...
function c57728571.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(c57728571.filter,1-tp,LOCATION_DECK,0,nil)
	if ct>0 and g:GetCount()>0 and Duel.SelectYesNo(1-tp,aux.Stringid(57728571,0)) then
		Duel.BreakEffect()
