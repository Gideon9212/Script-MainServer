--Parasite Fusioner
function c23114.initial_effect(c)
				...
				if gc then
					...
					while tc do
						if sub and tc:IsHasEffect(511002961) then
							...
							if bwg:IsExists(c23114.FConditionFilterCodeAndSub,1,nil,tc:GetCode(),e:GetHandler()) then
								...
								bwg:Remove(c23114.FConditionFilterCodeAndSub,nil,tc:GetCode(),e:GetHandler())
							else
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,gc,chkfnf)
				...
				if gc then
					...
					elseif gc:CheckFusionSubstitute(e:GetHandler()) and not fusubsel then
						sg:Remove(c23114.FConditionFilterx,nil,e:GetHandler(),code1,code2,code3)
						sg:RemoveCard(gc)
						fusubsel=true
					elseif fusubsel then sg:Remove(Card.IsFusionCode,nil,gc:GetCode())
					else sg:Remove(c23114.FConditionFilterCodeNotSub,nil,gc:GetCode(),e:GetHandler())
					end
					Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
					...
					else sg:Remove(c23114.FConditionFilterCodeNotSub,nil,g1:GetFirst():GetCode(),e:GetHandler())
					end
					Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
				...
				elseif g1:GetFirst():CheckFusionSubstitute(e:GetHandler()) and not fusubsel then
					...
					fusubsel=true
				elseif fusubsel then sg:Remove(Card.IsFusionCode,nil,g1:GetFirst():GetCode())
				else sg:Remove(c23114.FConditionFilterCodeNotSub,nil,g1:GetFirst():GetCode(),e:GetHandler()) end
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
				local g2=sg:Select(tp,1,1,nil)
				...
				else sg:Remove(c23114.FConditionFilterCodeNotSub,nil,g2:GetFirst():GetCode(),e:GetHandler()) end
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,gc,chkfnf)
				...
				if gc then
					...
					if gc:IsHasEffect(511002961) then
						sg:RemoveCard(gc)
					elseif gc:CheckFusionSubstitute(e:GetHandler()) and not fusubsel then
						sg:Remove(c23114.FConditionFilterx,nil,e:GetHandler(),code1,code2,code3,code4)
						sg:RemoveCard(gc)
						fusubsel=true
					elseif fusubsel then sg:Remove(Card.IsFusionCode,nil,gc:GetCode())
					else sg:Remove(c23114.FConditionFilterCodeNotSub,nil,gc:GetCode(),e:GetHandler())
				...
				else sg:Remove(c23114.FConditionFilterCodeNotSub,nil,g3:GetFirst():GetCode(),e:GetHandler()) end
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	...
	return	function(e,g,gc,chkfnf)
				...
				if gc then
					...
					if (gc:IsFusionCode(code) or (sub and gc:CheckFusionSubstitute(e:GetHandler()))) 
						and mg:IsExists(c23114.FConditionFilterConAndSub,cc,gc,f,sub) then
						return true
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,gc,chkfnf)
				...
				if gc then
					if (gc:IsFusionCode(code) or (sub and gc:CheckFusionSubstitute(e:GetHandler()))) 
						and g:IsExists(c23114.FConditionFilterConAndSub,cc,gc,f,sub) then
						Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
						local g1=g:FilterSelect(tp,c23114.FConditionFilterConAndSub,cc,cc,gc,f,sub)
						Duel.SetFusionMaterial(g1)
					else
	...
	return	function(e,g,gc,chkfnf)
				...
				if gc then
					if not gc:IsCanBeFusionMaterial(e:GetHandler()) then return false end
					return ((f1(gc) or gc:IsHasEffect(511002961)) and mg:IsExists(c23114.FConditionFilterConAndSub,1,gc,f2,true))
						or ((f2(gc) or gc:IsHasEffect(511002961)) and mg:IsExists(c23114.FConditionFilterConAndSub,1,gc,f1,true)) end
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,gc,chkfnf)
				...
				if gc then
					...
					if f1(gc) or gc:IsHasEffect(511002961) then sg:Merge(g:Filter(c23114.FConditionFilterConAndSub,gc,f2,true)) end
					if f2(gc) or gc:IsHasEffect(511002961) then sg:Merge(g:Filter(c23114.FConditionFilterConAndSub,gc,f1,true)) end
					Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
					local g1=sg:Select(tp,1,1,nil)
					Duel.SetFusionMaterial(g1)
					return
	...
	return	function(e,g,gc,chkfnf)
				...
				if gc then
					if not gc:IsCanBeFusionMaterial(e:GetHandler()) then return false end
					local bwxct=mg:FilterCount(c23114.FConditionFilterx3,gc,e:GetHandler(),sub)
					local bw=0
					local ct=mg:FilterCount(c23114.FConditionFilterCodeNotSub,gc,code,e:GetHandler())
					if mg:IsExists(c23114.FConditionFilterx2,1,gc,e:GetHandler(),sub) then bw=1 end
				...
				g1=g1:Filter(c23114.FConditionFilterCodeNotSub,nil,code,e:GetHandler())
				local g2=mg:Filter(c23114.FConditionFilterx2,nil,e:GetHandler(),sub)
				local g3=mg:Filter(c23114.FConditionFilterx3,nil,e:GetHandler(),sub)
				g1:Merge(g3)
				if chkf~=PLAYER_NONE then
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,gc,chkfnf)
				...
				local g=eg:Filter(Card.IsCanBeFusionMaterial,nil,e:GetHandler())
				if gc then
					local g1=g:Filter(c23114.FConditionFilterCodeNotSub,gc,code,e:GetHandler())
					local g3=g:Filter(c23114.FConditionFilterx3,gc,e:GetHandler(),sub)
					g1:Merge(g3)
						...
						if cc-1>0 then
							...
							for i=1,cc-1 do
								...
								if tc1:CheckFusionSubstitute(e:GetHandler()) and not tc1:IsHasEffect(511002961) then 
									sg:Remove(c23114.FConditionFilterx2,nil,e:GetHandler(),sub)
	...
	return	function(e,g,gc,chkfnf)
				...
				if gc then
					if not gc:IsCanBeFusionMaterial(e:GetHandler()) then return false end
					return (f(gc) or gc:IsHasEffect(511002961)) and mg:IsExists(c23114.FConditionFilterConAndSub,cc-1,gc,f,true) end
				local g1=mg:Filter(c23114.FConditionFilterConAndSub,nil,f,true)
				if chkf~=PLAYER_NONE then
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,gc,chkfnf)
				...
				if gc then
					Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
					local g1=g:FilterSelect(tp,c23114.FConditionFilterConAndSub,cc-1,cc-1,gc,f,true)
					Duel.SetFusionMaterial(g1)
					return
				end
				local sg=g:Filter(c23114.FConditionFilterConAndSub,nil,f,true)
	...
	function Auxiliary.FConditionFilterFFR(c,f1,f2,mg,minc,chkf)
		...
		if chkf==PLAYER_NONE or aux.FConditionCheckF(c,chkf) then
			return minc<=0 or mg:IsExists(c23114.FConditionFilterConAndSub,minc,c,f2,true)
		else
			local mg2=mg:Filter(c23114.FConditionFilterConAndSub,c,f2,true)
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,gc,chkfnf)
				...
				if aux.FConditionFilterFFR(gc,f1,f2,g,minct,chkf) then
					...
					if f2(gc) or gc:IsHasEffect(511002961) then
						...
						if mg1:GetCount()>0 then
							...
							local mg2=g:Filter(c23114.FConditionFilterConAndSub,nil,f2,true)
							mg1:Merge(mg2)
							if chkf~=PLAYER_NONE then
								Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
								...
								if not f2(sg:GetFirst()) and not sg:GetFirst():IsHasEffect(511002961) then
									if mg1:GetCount()>0 and maxct>0 and (minct>0 or Duel.SelectYesNo(tp,93)) then
										...
										Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
										local sg=mg1:FilterSelect(tp,c23114.FConditionFilterConAndSub,minct,maxct,nil,f2,true)
										g1:Merge(sg)
									end
									Duel.SetFusionMaterial(g1)
									return
							...
							if maxct>1 and (minct>1 or Duel.SelectYesNo(tp,93)) then
								Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
								local sg=mg1:FilterSelect(tp,c23114.FConditionFilterConAndSub,minct-1,maxct-1,nil,f2,true)
								g1:Merge(sg)
								mg1:Sub(sg)
					...
					local mg=g:Filter(c23114.FConditionFilterConAndSub,nil,f2,true)
					if chkf~=PLAYER_NONE then
						Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			...
			local mg=g:Filter(c23114.FConditionFilterConAndSub,g1:GetFirst(),f2,true)
	...
	e1:SetTarget(c23114.sptg)
	e1:SetOperation(c23114.spop)
	c:RegisterEffect(e1)
	--fusion substitute
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_FUSION_SUBSTITUTE)
	e2:SetCondition(c23114.subcon)
	c:RegisterEffect(e2)
...
function c23114.subcon(e)
	return e:GetHandler():IsFaceup() and e:GetHandler():IsOnField()
end
function c23114.filter1(c,e)
	return c:IsCanBeFusionMaterial() and not c:IsImmuneToEffect(e)
end
function c23114.filter2(c,e,tp,m,f,gc)
...
function c23114.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then
		local mg1=Duel.GetMatchingGroup(Card.IsCanBeFusionMaterial,tp,LOCATION_MZONE+LOCATION_HAND,0,c)
		local res=Duel.IsExistingMatchingCard(c23114.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,c)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c23114.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,c)
			end
		end
...
function c23114.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsFacedown() or not c:IsRelateToEffect(e) or c:IsImmuneToEffect(e) then return end
	local mg1=Duel.GetMatchingGroup(c23114.filter1,tp,LOCATION_MZONE+LOCATION_HAND,0,c,e)
	local sg1=Duel.GetMatchingGroup(c23114.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,c)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c23114.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,c)
	end
