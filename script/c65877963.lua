...
function c65877963.initial_effect(c)
	...
	e2:SetCost(c65877963.cost)
	e2:SetTarget(c65877963.target)
	e2:SetOperation(c65877963.operation)
	c:RegisterEffect(e2)
end
...
function c65877963.costfilter(c)
	return c:IsSetCard(0x117) and (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsAbleToGraveAsCost()
end
function c65877963.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c65877963.costfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c65877963.costfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,1,nil)
	Duel.SendtoGrave(g,REASON_COST)
end
function c65877963.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c65877963.operation(e,tp,eg,ep,ev,re,r,rp)
