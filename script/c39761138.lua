...
function c39761138.rmfilter(c)
	return c:IsAbleToRemove() and aux.SpElimFilter(c)
end
function c39761138.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE+LOCATION_GRAVE) and chkc:IsControler(1-tp) and c39761138.rmfilter(chkc) end
	if chk==0 then return true end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectTarget(tp,c39761138.rmfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,1,1,nil)
