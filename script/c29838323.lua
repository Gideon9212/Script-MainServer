...
function c29838323.desfilter(c,ft)
...
function c29838323.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsOnField() and chkc:IsControler(tp) and c29838323.desfilter(chkc,ft) end
	if chk==0 then
		return ft>-1 and Duel.IsExistingTarget(c29838323.desfilter,tp,LOCATION_ONFIELD,0,1,nil,ft)
			and Duel.IsExistingMatchingCard(c29838323.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c29838323.desfilter,tp,LOCATION_ONFIELD,0,1,1,nil,ft)
