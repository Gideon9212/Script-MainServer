...
function c511015107.filter(c,e,tp)
	...
	if not c:IsSetCard(0x10af) then return c:IsSynchroSummonable(nil)
		...
		mg:AddCard(mc)
		local res=mg:IsExists(c511015107.ddfilter,1,nil,c,mg)
		e1:Reset()
		e2:Reset()
		return res
	end
end
function c511015107.ddfilter(c,sc,mg)
	...
	if g:GetCount()>0 then
		...
		if not tc:IsSetCard(0x10af) then
			...
			if e:IsHasType(EFFECT_TYPE_ACTIVATE) and c:IsRelateToEffect(e) and (not tc:IsSynchroSummonable(nil,mg) or Duel.SelectYesNo(tp,93)) then
				...
				local smat=mg:FilterSelect(tp,c511015107.ddfilter,1,1,nil,tc,mg):GetFirst()
