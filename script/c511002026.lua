--Sphin-Quiz
function c511002026.initial_effect(c)
	--special summon
	...
	e1:SetCondition(c511002026.sumcon)
	e1:SetTarget(c511002026.sumtg)
	e1:SetOperation(c511002026.sumop)
	c:RegisterEffect(e1)
	...
	e2:SetCondition(c511002026.con)
	e2:SetValue(1)
	c:RegisterEffect(e2)
	--Quest
	...
	e3:SetCountLimit(1)
	e3:SetCondition(c511002026.qcon)
	e3:SetOperation(c511002026.qop)
	c:RegisterEffect(e3)
end
function c511002026.sumcon(e,tp,eg,ep,ev,re,r,rp)
	local rc=eg:GetFirst()
	return rc:IsSetCard(0x541)
end
function c511002026.sumtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c511002026.sumop(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():IsRelateToEffect(e) then
		Duel.SpecialSummon(e:GetHandler(),0,tp,tp,false,false,POS_FACEUP)
	end
end
function c511002026.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x541)
end
function c511002026.con(e)
	return Duel.IsExistingMatchingCard(c511002026.cfilter,e:GetHandlerPlayer(),LOCATION_MZONE,0,1,e:GetHandler())
end
function c511002026.qcon(e,tp,eg,ep,ev,re,r,rp)
	local bt=eg:GetFirst()
	return r~=REASON_REPLACE and bt:IsSetCard(0x541)
end
function c511002026.qop(e,tp,eg,ep,ev,re,r,rp)
