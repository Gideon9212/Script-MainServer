--Past Tuning
function c511003052.initial_effect(c)
	--Activate
	...
	e1:SetTarget(c511003052.target)
	e1:SetOperation(c511003052.activate)
	c:RegisterEffect(e1)
end
function c511003052.ccfilter(c,mc,oc,lv,tp)
...
function c511003052.ccfilter2(c)
	return c:IsFaceup() and c:IsCode(511003050)
end
function c511003052.filter(c,mc,tp)
	local lv=c:GetLevel()
	local clv=0
	...
	return c:IsFaceup() and lv>0 and Duel.IsExistingMatchingCard(c511003052.ccfilter,tp,LOCATION_MZONE,0,1,nil,mc,c,clv,tp)
end
function c511003052.lvfilter(c)
	return c:IsFaceup() and c:GetLevel()>0
end
function c511003052.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and c511003052.lvfilter(chkc) end
	local c=e:GetHandler()
	if chk==0 then return e:IsHasType(EFFECT_TYPE_ACTIVATE) 
		and Duel.IsExistingTarget(c511003052.filter,tp,0,LOCATION_MZONE,1,nil,c,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c511003052.filter,tp,0,LOCATION_MZONE,1,1,nil,c,tp)
end
function c511003052.activate(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	...
	if tc and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		Duel.BreakEffect()
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
		local g=Duel.SelectMatchingCard(tp,c511003052.ccfilter,tp,LOCATION_MZONE,0,1,1,nil,c,tc,clv,tp)
		if g:GetCount()<=0 then
			g=Duel.SelectMatchingCard(tp,c511003052.ccfilter2,tp,LOCATION_MZONE,0,1,1,nil)
		...
		if syng:GetCount()>0 then
			...
			sc:RegisterFlagEffect(51103052,RESET_EVENT+0x1fe0000,0,1,fid)
			...
			e16:SetCondition(c511003052.descon)
			e16:SetOperation(c511003052.desop)
			e16:SetReset(RESET_PHASE+PHASE_END)
			Duel.RegisterEffect(e16,tp)
		end
	end
end
function c511003052.descon(e,tp,eg,ep,ev,re,r,rp)
...
function c511003052.desop(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject()
