...
os = require('os')
function c511002782.initial_effect(c)
	--flip
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetDescription(aux.Stringid(51102781,1))
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_FLIP)
	e1:SetOperation(c511002782.operation)
	c:RegisterEffect(e1)
end
function c511002782.spfilter(c,e,tp)
	return c:IsCode(511001940) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c511002782.operation(e,tp,eg,ep,ev,re,r,rp)
	local endtime=0
	local check=true
	...
	local dn=0
	repeat
		Duel.Hint(HINT_SELECTMSG,1-tp,aux.Stringid(4006,0))
