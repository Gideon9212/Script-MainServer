...
function c511000865.filter(c,e,tp)
	return c:IsFaceup() and c:IsType(TYPE_SYNCHRO) and c:IsAbleToExtra() and Duel.GetLocationCountFromEx(tp,tp,c)>0 
		and Duel.IsExistingMatchingCard(c511000865.spfilter,tp,LOCATION_EXTRA,0,1,nil,c:GetLevel(),c:GetCode(),e,tp)
end
...
function c511000865.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoDeck(tc,nil,0,REASON_EFFECT)~=0 and Duel.GetLocationCountFromEx(tp)>0 then
		Duel.BreakEffect()
		...
		if g and Duel.SpecialSummonStep(g,0,tp,tp,false,false,POS_FACEUP)~=0 then
