...
function c16255173.initial_effect(c)
	...
	c:RegisterEffect(e1)
	aux.AddEREquipLimit(c,nil,function(ec,_,tp) return ec:IsControler(1-tp) end,c16255173.equipop,e1)
...
function c16255173.equipop(c,e,tp,tc)
	local atk=tc:GetTextAttack()
	if tc:IsFacedown() or atk<0 then atk=0 end
	if not aux.EquipByEffectAndLimitRegister(c,e,tp,tc,16255173,true) then return end
	local e1=Effect.CreateEffect(c)
...
function c16255173.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not (tc and tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsType(TYPE_MONSTER)) then return end
	if c:IsFaceup() and c:IsRelateToEffect(e) then
		c16255173.equipop(c,e,tp,tc)
