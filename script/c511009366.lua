...
function c511009366.checkop(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		if tc:IsFaceup() and tc:IsType(TYPE_MONSTER) then
			tc:RegisterFlagEffect(511009366,RESET_EVENT+0x1fe0000,0,1)
		end
		tc=eg:GetNext()
	end
end
...
function c511009366.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c511009366.thfilter,tp,LOCATION_DECK,0,1,1,nil)
...
function c511009366.eftg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c511009366.effilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c511009366.effilter,tp,LOCATION_MZONE,0,1,nil) end
...
function c511009366.efop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsFaceup() and tc:IsRelateToEffect(e) and not tc:IsImmuneToEffect(e) and Duel.Release(tc,REASON_EFFECT)>0 then
		c:RegisterFlagEffect(tc:GetCode(),RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
