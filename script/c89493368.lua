...
function c89493368.rescon(ct)
	return	function(sg,e,tp,mg)
				return aux.ChkfMMZ(1-ct)(sg,e,tp,mg) and sg:IsExists(c89493368.chk,1,nil,sg)
			end
end
function c89493368.chk(c,sg)
	return c:IsCode(15175429) and sg:IsExists(Card.IsCode,1,c,52286175)
end
function c89493368.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local g1=Duel.GetMatchingGroup(c89493368.spcfilter,tp,LOCATION_ONFIELD,0,c,15175429)
	local g2=Duel.GetMatchingGroup(c89493368.spcfilter,tp,LOCATION_ONFIELD,0,c,52286175)
	local g=g1:Clone()
	g:Merge(g2)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>-3 and e:GetHandler():IsAbleToGraveAsCost()
		and g1:GetCount()>0 and g2:GetCount()>0 and aux.SelectUnselectGroup(g,e,tp,2,2,c89493368.rescon(dec),0) end
	local sg=aux.SelectUnselectGroup(g,e,tp,2,2,c89493368.rescon(dec),1,tp,HINTMSG_TOGRAVE)
	sg:AddCard(c)
