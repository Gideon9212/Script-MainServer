...
function c75347539.rescon(sg,e,tp,mg)
	return aux.ChkfMMZ(1)(sg,e,tp,mg) and sg:IsExists(c75347539.chk,1,nil,sg,Group.CreateGroup(),99785935,39256679,11549357)
end
function c75347539.chk(c,sg,g,code,...)
	if not c:IsCode(code) then return false end
	local res
	if ... then
		g:AddCard(c)
		res=sg:IsExists(c75347539.chk,1,g,sg,g,...)
		g:RemoveCard(c)
	else
		res=true
	end
...
function c75347539.spcon(e,c)
	...
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-3 and g1:GetCount()>0 and g2:GetCount()>0 and g3:GetCount()>0 
		and aux.SelectUnselectGroup(g,e,tp,3,3,c75347539.rescon,0)
end
function c75347539.spop(e,tp,eg,ep,ev,re,r,rp,c)
	...
	local g=aux.SelectUnselectGroup(rg,e,tp,3,3,c75347539.rescon,1,tp,HINTMSG_RELEASE)
