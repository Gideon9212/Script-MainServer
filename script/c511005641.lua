...
function c511005641.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE+LOCATION_GRAVE) and chkc:IsControler(1-tp) and c511005641.rfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c511005641.rfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,1,nil) end
	local ct=e:GetLabel()
	e:SetLabel(0)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectTarget(tp,c511005641.rfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,ct,ct,nil)
