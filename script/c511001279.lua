...
function c511001279.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE+LOCATION_GRAVE) and chkc:IsControler(tp) and c511001279.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c511001279.filter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,2,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectTarget(tp,c511001279.filter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,2,2,nil)
