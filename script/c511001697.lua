...
function c511001697.filter(c,tp)
	local re=c:GetReasonEffect()
	return (Duel.GetLocationCount(tp,LOCATION_MZONE)>0 or c:GetSequence()<5) and c:GetLevel()==3 and c:IsSetCard(0xe6)
...
function c511001697.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c511001697.filter,1,nil,tp) end
	local g=Duel.SelectReleaseGroup(tp,c511001697.filter,1,1,nil,tp)
