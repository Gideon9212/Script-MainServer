...
function c511015112.pfusfilter2(c,e,tp,m,f)
...
function c511015112.pfustg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local res=Duel.IsExistingMatchingCard(c511015112.pfusfilter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c511015112.pfusfilter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf)
			end
		end
		return res
	end
...
function c511015112.pfusop(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg1=Duel.GetMatchingGroup(c511015112.pfusfilter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c511015112.pfusfilter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf)
	end
...
function c511015112.mfusop(e,tp,eg,ep,ev,re,r,rp)
	...
	if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
		...
		local tc=tg:GetFirst()
		if sg1:IsContains(tc) and (sg2==nil or not sg2:IsContains(tc) or not Duel.SelectYesNo(tp,ce:GetDescription())) then
			local mat1=mg1:FilterSelect(tp,c511015112.mfusfilter3,1,1,nil,c,tc,tp)
			mat1:AddCard(c)
			tc:SetMaterial(mat1)
			Duel.SendtoGrave(mat1,REASON_EFFECT+REASON_MATERIAL+REASON_FUSION)
			Duel.BreakEffect()
			...
			local mat2=mg2:FilterSelect(tp,c511015112.mfusfilter3,1,1,nil,c,tc,tp)
			mat1:AddCard(c)
			local fop=ce:GetOperation()
			fop(ce,e,tp,tc,mat2)
		end
